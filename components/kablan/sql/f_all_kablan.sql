CREATE OR REPLACE FUNCTION f_all_kablan(prjId INTEGER, kablanId INTEGER DEFAULT NULL) RETURNS TABLE (
        kablan_id INTEGER,
        kablan_name TEXT,
        price_tasks NUMERIC,
...
SELECT u.id::INTEGER AS kablan_id,
...
WHERE u.role = 'KABLAN'
    AND (
        kablanId IS NULL
        OR u.id = kablanId
    ) -- Apply filtering if kablanId is provided
GROUP BY u.id,
    u.name,
    p.price_bgt_reqs,
    p.total_granted_bgt_reqs;
END;
$$ LANGUAGE plpgsql;
