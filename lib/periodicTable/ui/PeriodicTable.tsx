...
import SelectFront from './periodicFilter/SelectFront'
import PeriodicFilter from './periodicFilter'
import { Btn } from 'zvijude/btns'

export default function PeriodicTable({ qrsData, prjId, query, fields, parts }) {
  const floors = Object.keys(qrsData)
    .map(Number)
    .sort((a, b) => b - a)
  return (
    <div className='p-8 rounded-md bg-white shadow mobile:p-4'>
      <PeriodicFilter fields={fields} query={query} parts={parts} />
      ...
      <div className='grid gap-4'>
        {floors.map((floor) => (
          <div key={floor}>
            {/* Floor label column */}
            <p className='text-lg mobile:text-base font-semibold text-gray-800 mb-1'>
              קומה {floor}
            </p>
            ...
            <div className='flex flex-wrap gap-2 mobile:gap-1'>
              {Object.keys(qrsData[floor]).map((aptNum) => (
                <div
                  key={aptNum}
                  className='border p-2 mobile:p-1 rounded-md flex flex-col gap-2 mobile:gap-1 items-center'>
