...
export default function Filter({ query, fields }) {
  ...
  function resetFilter() {
    ...
    document.getElementById('filterSkippedTasks')?.hidePopover()
  }

  return (
    <div popover='auto' className='pop max-h-[75vh] w-[560px] scroll-bar overflow-y-auto' id='filterSkippedTasks'>
      ...
      <form id='filterForm' onSubmit={onSubmit}>
        <section className='grid grid-cols-2 gap-4'>
          ...
          <Select
            ...
            placeholder='כולם...'
            defaultValue={query?.skippedBy?.name}
            required={false}
          />
          ...
          <div>
            ...
            <Input
              ...
              name='fromDate_s'
              defaultValue={
                query?.skippedAt?.gte ? new Date(query?.skippedAt?.gte).toISOString().split('T')[0] : ''
              }
              required={false}
            />
            <Input
              ...
              name='toDate_s'
              defaultValue={
                query?.skippedAt?.lte ? new Date(query?.skippedAt?.lte).toISOString().split('T')[0] : ''
              }
              required={false}
            />
          </div>
