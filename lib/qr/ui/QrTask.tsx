...
export function QrTask({ user, qrData, aptOpt }) {
  ...
  return (
    <>
      <div className='grid place-items-center gap-4'>
        <QrHeader userRole={user.role} qrData={qrData} aptOpt={aptOpt} />
      ...
      <TaskCompletionForm
        curTask={curTask}
        bgtReqByStatus={bgtReqByStatus}
        probsByStatus={probsByStatus}
      />
    </>
  )
}
    ...
    <div className='w-full p-2 px-5 rounded shadow-3'>
      ...
      {!isWaitingForApproval && Boolean(curTask.needApproval || curTask.media) && (
        <div className='my-1 py-2 text-xs text-gray-700 font-semibold border-t border-gray-200'>
          ...
          {curTask.isMedia && <p>* חובה להעלות תמונה או סרטון בסיום המשימה</p>}
        </div>
      )}
    </div>
  )
